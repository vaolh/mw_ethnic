...
*** REPLICATION FILE: did-indlevel-inc-year.do
*** STATA VERSION:    StataNow 19.5
*** AUTHORS:          Matías Carrasco, Victor Ortega Le Hénanff
*** DATE:             2026-05-06

*** All individual-level YEAR-cadence DiD tables in one file.
*** Output (in paper/tables/):
***   1. did-wage-ethnic-year.tex          (Topic A: log wages by ethnicity)
***   2. did-income-ethnic-year.tex        (Topic B: log income by ethnicity)
***   3. did-wage-deciles-year.tex         (Topic C: heterogeneity by income decile × formal/informal)
***   4. did-wage-informality-year.tex     (Topic D: formal / informal / self-emp shares)
***   5. did-wage-sector-year.tex          (Topic E: covered / uncovered sectors)
***   6. sum-sector-shares-year.tex        (Topic E supplement: pre-treat sector distribution)
***
*** Reads: code/estimate/read-indlevel-inc-year.do
***        code/estimate/_helpers.do (exporttable)

cap mkdir log
log using "log/did-indlevel-inc-year.log", replace text
...
*** Synthesize convenience names used by Topics D & E.
*** Year build provides indep1/indep2 and ind1; year build also provides
*** `informal` directly from the CONEVAL merge.
...
esttab a1 a2 a3 a4 a5 using "`TABDIR'did-wage-ethnic-year.tex", replace label fragment ///
...
esttab b1 b2 b3 b4 b5 using "`TABDIR'did-income-ethnic-year.tex", replace label fragment ///
    nolines posthead(\cmidrule{2-6}) prefoot(\midrule)                                    ///
    postfoot(\bottomrule \bottomrule) booktabs                                            ///
    nonumbers mtitle("(1)" "(2)" "(3)" "(4)" "(5)") collabels(none)                       ///
    cells(b(star fmt(%9.3f)) se(par fmt(%9.3f)))                                          ///
...
esttab c1 c2 c3 using "`TABDIR'did-wage-deciles-year.tex", replace label fragment ///
...
local INFOROUT "`TABDIR'did-wage-informality-year.tex"
...
*** Pre-treatment sector shares (supplement table)
...
esttab share_all share_nonind share_ind using "`TABDIR'sum-sector-shares-year.tex", ///
...
local SECTOROUT "`TABDIR'did-wage-sector-year.tex"
