...
*** REPLICATION FILE: enigh-hhlevel-inc-month.do
*** STATA VERSION:    StataNow 19.5
*** AUTHORS:          Matías Carrasco, Victor Ortega Le Hénanff
*** DATE:             2026-05-06

*** Builds the household × month ENIGH income panel.
*** Each row = one household per calendar month within the survey reference
*** period (typically Feb–Sep of each ENIGH wave).
***   - HH income aggregated from enigh-indlevel-inc-month (sum across earners)
***   - HH composition aggregated from enigh-indlevel-inc-month rows in that month
***   - HH-invariant attributes from concentradohogar (replicated to each month)
***
*** Output: ../../data/clean/enigh/enigh-hhlevel-inc-month.dta
*** Companion R script: enigh-hhlevel-inc-month.R

cap mkdir log
log using "log/enigh-hhlevel-inc-month.log", replace text
...
foreach yr of local YEARS {
    ...
    *** in enigh-hhlevel-inc-year.dta which uses the full quarterly aggregate).
...
compress
save "../../data/clean/enigh/enigh-hhlevel-inc-month.dta", replace

display _n "Saved enigh-hhlevel-inc-month.dta with " _N " HH-month obs and " c(k) " variables."
