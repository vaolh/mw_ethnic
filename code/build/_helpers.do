...
*** ENIGH expenditure clave codes have TWO incompatible schemes across waves:
***   pre-2024 (2016, 2018, 2020, 2022): 6-digit numeric strings (e.g.
***       "011131" = Tortillas de maíz). First 2 digits = legacy capítulo.
***   2024:    alpha-prefixed strings (e.g. "A001" = Maíz en grano, "B001"
***       = Metro o tren ligero). Letter does NOT map to capítulo
***       one-for-one; e.g. letter A spans food + alcohol + tobacco +
***       restaurants depending on the numeric suffix.
***
*** The previous classify_gasto_clave applied `substr(clave, 1, 2) == "01"`
*** etc., which silently classified ALL 2024 rows as gasto_group = "other"
*** (because substr("A001", 1, 2) = "A0", never matching numeric branches).
***
*** Harmonized classification now uses a manually-authored crosswalk:
***   data/clean/enigh/clave_subgroups_seed.csv  — range-based seed rules
***   data/clean/enigh/clave_crosswalk.csv — generated by
***       code/build/build-clave-crosswalk.py from the seed + the label
***       dictionary, mapping every clave (numeric or alpha) to:
***         capitulo_norm   — harmonized 2-digit code
***         subgroup        — fine-grained concepto bucket (~65 categories)
***         subgroup_label  — Spanish display label
***
*** T-codes (T901..T916) are pre-aggregated totals — drop them upstream.
***
*** Capitulo groups (harmonized, 2-digit):
...
***   12 = Cuidados personales y seguros (incluye seguros no médicos)
***   13 = Servicios diversos (profesionales, funerales, eventos, hogar)
***   18 = Otros productos / sin clasificar

cap program drop classify_gasto_clave
program define classify_gasto_clave
    *** Adds four columns to the gastos data in memory by merging against
    *** the precomputed crosswalk at data/clean/enigh/clave_crosswalk.dta
    *** (built by code/build/build-clave-crosswalk.do, which enumerates
    *** every unique clave across gastoshogar*.dta and applies the seed
    *** rules from data/clean/enigh/clave_subgroups_seed.csv).
    ***
    *** Output columns:
    ***   capitulo       — 2-digit harmonized capítulo code
    ***   subgroup       — fine-grained concepto bucket (~65 categories)
    ***   subgroup_label — Spanish display label
    ***   gasto_group    — coarse 14-bucket group derived from capitulo
    ***                    (preserved for back-compat with existing scripts)
    cap drop capitulo
    cap drop capitulo_norm
    cap drop subgroup
    cap drop subgroup_label
    cap drop gasto_group

    *** Drop pre-aggregated T-codes so we don't double-count.
    drop if substr(clave, 1, 1) == "T"

    qui merge m:1 clave using "../../data/clean/enigh/clave_crosswalk.dta", ///
        keep(master match) nogen

    *** Loud failure if any clave isn't in the crosswalk — likely means a
    *** newly-encountered code; rerun build-clave-crosswalk.do and
    *** extend data/clean/enigh/clave_subgroups_seed.csv if needed.
    cap assert !missing(subgroup)
    if _rc {
        di as error "classify_gasto_clave: clave not in crosswalk; " ///
                    "re-run code/build/build-clave-crosswalk.do"
        tab clave if missing(subgroup)
        exit 9
    }

    rename capitulo_norm capitulo

    *** Back-compat: derive the 14-bucket gasto_group from capitulo.
